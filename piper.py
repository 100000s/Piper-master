...
# load a blank image of the paper wallet with no QR codes or keys on it
...
def genKeys(remPubKey, remPrivKey, password, coinType, remPW, bip0032):
    ...
    if bip0032:
        ...
        # this actually generates the keys.  see the file genkeys.py or
        # genkeys_forget.py
        ...
        if (password != ""):
            privkey = WalletEnc.pw_encode(
                btckeys.pubkey, btckeys.privkey, password)
            sqliteEncrypted = 1
        else:
            privkey = btckeys.privkey
    ...
    if rememberKeys:
        ...
        try:
            ...
            con.execute(
                "INSERT INTO keys (serialnum, public, private, btcaddr, coinType, password, encrypted) VALUES (?,?,?,?,?,?,?)",
                (snum,
                 sqlitePubKey,
                 sqlitePrivKey,
                 sqliteBtcAddr,
...
def genAndPrintKeys(
    ...
    pubkey, privkey, leftMarkText, btcaddr = genKeys(
        remPubKey, remPrivKey, password, coinType, remPW, (keyGenType == 'bip0032'))
...
def genAndPrintKeysAndPass(
    ...
    pubkey, privkey, leftMarkText, btcaddr = genKeys(
        remPubKey, remPrivKey, password, coinType, remPW, False)
