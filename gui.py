...
class PiperUtil:
    def reprintPassword(self, event):
        ...
        Piper.print_password("Serial Number: " +
                             str(event.widget.serialnum), event.widget.password)
    ...
    def printBulkWallet(self):
        ...
        try:
            ...
            qrPopup = QRPopup()
            qrPopup.showAlert(
                "Error, enter an integer for the number of wallets field!")
            raise
    ...
    def printWallet(self):
        ...
        try:
            ...
            if (printPW == 1):
                Piper.genAndPrintKeysAndPass(
                    remFlag, remPrivFlag, qty, tab1.walletPass.get(), coinType, remPW)
            elif (printPW == 2):
            ...
            qrPopup = QRPopup()
            qrPopup.showAlert(
                "Error, enter an integer for the copies of wallet field!")
    ...
    def reprint(self, event):
        ...
        Piper.print_keypair(event.widget.pubkey,
                            event.widget.privkey,
                            "Serial Number: " + str(event.widget.serialnum),
                            event.widget.coinType)

    def showPassword(self, event):
        self.showMessage(
            "Password of encrypted private key",
            event.widget.password)
...
class DecryptedKeyPopup:
    ...
    def decryptKey(self):
        # try:
        self.decryptedKeyOnPopup.set(
            wallet_enc.decryptBIP0038(
                self.encryptedPrivKey,
                self.walletPass.get()))
        """
		except:
    ...
    def showPopup(self, event):
        ...
        tk.Label(
            passFrame,
            text='Enter password: ').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        tk.Entry(passFrame, width=10, textvariable=self.walletPass,
                 show="*").pack(side=tk.LEFT, padx=10, pady=10)
        passFrame.pack()
        ...
        msg = tk.Entry(
            resultFrame,
            textvariable=self.decryptedKeyOnPopup,
            width=52)
...
class QRPopup:
    ...
    def show2CodePopup(self, widget):
        ...
        font = ImageFont.truetype(
            "/usr/share/fonts/ttf/ubuntu-font-family-0.80/UbuntuMono-R.ttf", 14)
        draw = ImageDraw.Draw(finalImg)
        draw.text((10, 500), "Public: " +
                  widget.pubkey, font=font, fill=(0, 0, 0))
        ...
        while (textSize[0] > 400):
            ...
            draw.text((600, 360 + (lineInd * 15)),
                      textInner, font=font, fill=(0, 0, 0))

            textSize = draw.textsize(textToPrint[charInd:], font=font)
            lineInd += 1

        draw.text((600, 360 + (lineInd * 15)),
                  textToPrint[charInd:], font=font, fill=(0, 0, 0))
...
class Tab1(tk.Frame):
    ...
    def __init__(self, root):
        ...
        tk.Label(
            qtyFrame,
            text='Number of copies of wallet to print ').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        qtyField = tk.Entry(
            qtyFrame,
            width=10,
            textvariable=self.walletQuantity)
        ...
        tk.Label(
            rdoFrame,
            text='Remember public key ').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        remPubKeyBtn = tk.Radiobutton(
            rdoFrame, text="Yes", variable=self.remKey, value=1)
        remPubKeyBtn.pack(side=tk.LEFT)
        remPubKeyBtn.select()
        tk.Radiobutton(
            rdoFrame,
            text="No",
            variable=self.remKey,
            value=2).pack(
            side=tk.LEFT)
        rdoFrame.pack()
        ...
        tk.Label(
            rdoFrame2,
            text='Remember private key ').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        remPrivKeyBtn = tk.Radiobutton(
            rdoFrame2, text="Yes", variable=self.remPrivKey, value=1)
        remPrivKeyBtn.pack(side=tk.LEFT)
        remPrivKeyBtn.select()
        tk.Radiobutton(
            rdoFrame2,
            text="No",
            variable=self.remPrivKey,
            value=2).pack(
            side=tk.LEFT)
        rdoFrame2.pack()
        ...
        tk.Radiobutton(
            self.rdoFrame3,
            text="Manual Entry",
            variable=self.encKey,
            value=1,
            command=pUtil.setEncryptYes).pack(
            side=tk.LEFT)
        tk.Radiobutton(
            self.rdoFrame3,
            text="Automatic",
            variable=self.encKey,
            value=3,
            command=pUtil.setAutomaticEncryptYes).pack(
            side=tk.LEFT)
        encNoBtn = tk.Radiobutton(
            self.rdoFrame3,
            text="No",
            variable=self.encKey,
            value=2,
            command=pUtil.setEncryptNo)
        encNoBtn.pack(side=tk.LEFT)
        encNoBtn.select()
        self.rdoFrame3.pack()

        self.passGroup = tk.LabelFrame(
            printGroup, text="Encrypted wallet password")
        ...
        tk.Label(
            verFrame,
            text='Verify password: ').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        tk.Entry(verFrame, width=10, textvariable=self.verPass,
                 show="*").pack(side=tk.LEFT, padx=10, pady=10)
        ...
        tk.Label(
            rdoFrame4,
            text='Print password').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        printPasswdBtn = tk.Radiobutton(
            rdoFrame4, text="Yes", variable=self.printPW, value=1)
        printPasswdBtn.pack(side=tk.LEFT)
        printPasswdBtn.select()
        tk.Radiobutton(
            rdoFrame4,
            text="No",
            variable=self.printPW,
            value=2).pack(
            side=tk.LEFT)
        rdoFrame4.pack()
        ...
        tk.Label(
            self.remPWFrame,
            text='Remember password ').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        remPWBtn = tk.Radiobutton(
            self.remPWFrame,
            text="Yes",
            variable=self.remPW,
            value=1)
        ...
        self.printButton = tk.Button(
            printGroup, text='Print', command=pUtil.printWallet)
...
class Tab2(tk.Frame):
    def __init__(self, root):
        ...
        tk.Label(
            bulkQtyFrame,
            text='Number of wallets to print using settings from Print Wallet tab ').pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        tk.Entry(
            bulkQtyFrame,
            width=10,
            textvariable=self.bulkQuantity).pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        printBulkButton = tk.Button(
            bulkGroup,
            text="Print Bulk",
            command=pUtil.printBulkWallet)
...
class Tab3(tk.Frame):
    def __init__(self, root):
        ...
        self.viewKeysScrollbar = tk.Scrollbar(
            self, orient="vertical", command=self.viewKeysCanvas.yview)
        self.viewKeysCanvas.configure(
            yscrollcommand=self.viewKeysScrollbar.set)
        self.viewKeysScrollbar.pack(side="right", fill="y")
        self.viewKeysCanvas.pack(fill="both", expand=True)
        self.viewKeysCanvas.create_window(
            0, 0, window=self.viewKeysGroup, anchor="nw")
    ...
    def populate(self):
        ...
        try:
            ...
            for row in rows:
                aKeyGroup = tk.LabelFrame(
                    self.viewKeysGroup, text="Serial num: " + str(row[0]))
    ...
    def on_frame_configure(self, event):
        ...
        self.viewKeysCanvas.configure(
            scrollregion=self.viewKeysCanvas.bbox("all"))
...
class Tab4(tk.Frame):
    ...
    def populate(self):
        ...
        label1 = tk.Label(
            self,
            text="Greater heat time means a darker print.  The default value for 20 year paper is 100.")
        ...
        tk.Label(
            pFwareFrame,
            text="Printer firmware version:").pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        tk.Radiobutton(
            keyGenTypeFrame,
            text="Traditional",
            variable=self.keyGenType,
            value="trad",
            command=pUtil.setTradYes).pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        tk.Radiobutton(
        ...
        tk.Entry(
            addressPrefixFrame,
            textvariable=self.addrPrefix).pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        tk.Label(
            self.tradFrame,
            text="Address prefix must begin with 1 for bitcoin and L for litecoin or key generation will fail.").pack(
            padx=10)
        tk.Label(
            self.tradFrame,
            text="We do not recommend a prefix longer than 3 characters.  ").pack(
            padx=10)
        tk.Label(
            self.tradFrame,
            text="A prefix just 3 characters long can take up to 30 seconds to generate (for example 1CC).").pack(
            padx=10)

        headlessEncryptFrame = tk.Frame(self.tradFrame)

        tk.Label(
            headlessEncryptFrame,
            text="Automatically encrypt private key:").pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        tk.Label(
            randPwLengthFrame,
            text='Automatic password word length:').pack(
            side=tk.LEFT,
            padx=10,
            pady=0)
        self.randPwLength = tk.Scale(
            randPwLengthFrame,
            from_=1,
            to=16,
            length=255,
    ...
    def applySettings(self):
        ...
        if (len(self.addrPrefix.get()) ==
                0 or self.addrPrefix.get()[0] != defaultPrefix):
            self.showMessage(
                "Error!!",
                "Invalid prefix.  " +
                self.coinType.get() +
                " addresses must begin with " +
                defaultPrefix)
            return

        process = Popen(["./vanitygen",
                         "-q",
                         "-n",
                         "-t",
                         "1",
        ...
        if (prefixValid == False):
            self.showMessage(
                "Error!!",
                "Invalid address prefix.  Not all characters are valid.")
            return

        try:
            con.execute(
                "UPDATE Settings SET value=? WHERE key='heatTime';", (str(
                    self.heatTime.get()),))
            con.execute(
                "UPDATE Settings SET value=? WHERE key='cointype';", (self.coinType.get(),))
            con.execute(
                "UPDATE Settings SET value=? WHERE key='addrPrefix';",
                (self.addrPrefix.get(),
                 ))
            # con.execute("UPDATE Settings SET value=? WHERE key='encType';", (self.encType.get(),))
            con.execute(
                "UPDATE Settings SET value=? WHERE key='headlessEnc';",
                (self.headlessEnc.get(),
                 ))
            con.execute(
                "UPDATE Settings SET value=? WHERE key='randomPasswordLength';",
                (self.randPwLength.get(),
                 ))
            con.execute(
                "UPDATE Settings SET value=? WHERE key='keyGenType';",
                (self.keyGenType.get(),
                 ))
            con.execute(
                "UPDATE Settings SET value=? WHERE key='printerFirmware';",
                (self.pFware.get(),
                 ))
        except sqlite3.Error as e:
            print("Error %s:" % e.args[0])
...
class Tab5(tk.Frame):
    def __init__(self, root):
        ...
        tk.Entry(
            textBoxFrameSSSS,
            textvariable=self.SSSStextBox).pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        opMenu = tk.OptionMenu(*(ddFrame, self.k) + tuple(options))
        opMenu.pack(side=tk.LEFT, padx=10, pady=10)
        self.k.set("3")
        tk.Label(ddFrame, text=" of ").pack(side=tk.LEFT)

        opMenuN = tk.OptionMenu(*(ddFrame, self.n) + tuple(options))
        opMenuN.pack(side=tk.LEFT, padx=10, pady=10)
        self.n.set("5")
        tk.Label(
            ddFrame,
            text="parts to reconstruct the secret").pack(
            side=tk.LEFT,
            padx=10)
        ...
        printButtonSSSS = tk.Button(
            buttonHolderSSSS,
            text="Print shares",
            command=self.splitAndPrint)
        ...
        tk.Entry(
            textBoxFrame,
            textvariable=self.textBox).pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        textBoxFrame.pack()
        ...
        printAsTextButton = tk.Button(
            buttonHolder,
            text="Print as text",
            command=self.printAsText)
        printAsTextButton.pack(side=tk.LEFT, padx=10, pady=10)

        printAsQRButton = tk.Button(
            buttonHolder,
            text="Print as QR code",
            command=self.printAsQR)
        ...
        AddAltButton = tk.Button(
            AddAltFrame,
            text="Add Alt Coin",
            command=self.launchAddAlt)
...
class AltCoinMan(tk.Toplevel):
    def __init__(self):
        ...
        tk.Entry(
            self.coinNameFrame,
            textvariable=self.newCoinName).pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        tk.Label(
            self.coinNameHolder,
            text="This is the name of the alt-coin that you're adding.  Bitcoin, Litecoin, Dogecoin etc.").pack(
            padx=10)
        ...
        tk.Label(
            self,
            text="This is the address prefix for the coin's public addresses.").pack(
            padx=10)
        tk.Label(
            self,
            text="On Bitcoin, this is a 1.  Dogecoin is D, and Litecoin is L.").pack(
            padx=10)

        versionNumFrame = tk.Frame(self)
        tk.Label(
            versionNumFrame,
            text="Version Code").pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        tk.Entry(
            versionNumFrame,
            textvariable=self.versionNum).pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        tk.Label(
            bgImgFrame,
            text="Wallet Background Image").pack(
            side=tk.LEFT,
            padx=10,
            pady=10)
        ...
        tk.Label(
            self,
            text="This is the prefix for the image filename of the wallet background image.").pack(
            padx=10)
        tk.Label(
            self,
            text="If you're not sure what to put here, put the word blank.").pack(
            padx=10)
        tk.Label(
            self, text="The image files live in /home/pi/Printer/Images.").pack(padx=10)
