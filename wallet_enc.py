# this code was mostly taken and modified from the electrum project
# located here: https://github.com/spesmilo/electrum
...
def EncodeAES(secret, s): return b58encode(aes.encryptData(secret, s))
def DecodeAES(secret, e): return aes.decryptData(secret, b58decode(e))
...
def encryptBIP0038(pubkey, privkey, secret):
    k = CKey()
    # decode the wallet import format by base58 decoding then dropping the
    # last 4 bytes and the first byte
...
def pw_encode(pub, priv, password):
    if password:
        ...
        if encType == "aes":
            ...
            # do not pass in secret, pass in password, because the BIP0038 spec
            # doesn't specify that you should hash passwords first in this way
            # like with AES
